...
// CSV Data Steps
Given(
  "I load test data from CSV file {string}",
  async function (filePath: string) {
    ...
    try {
      ...
      logger.error(
        `❌ Failed to load CSV file: ${error?.message || "Unknown error"}`
      );
      throw error;
    }
  }
...
// Excel Data Steps
Given(
  "I load test data from Excel file {string}",
  async function (filePath: string) {
    ...
    try {
      ...
      logger.error(
        `❌ Failed to load Excel file: ${error?.message || "Unknown error"}`
      );
      throw error;
    }
  }
...
Given("I have Excel data with multiple test types", async function () {
  ...
  currentData = await testManager.loadTestData(
...
// JSON Data Steps
Given(
  "I load test data from JSON file {string}",
  async function (filePath: string) {
    ...
    try {
      ...
      logger.error(
        `❌ Failed to load JSON file: ${error?.message || "Unknown error"}`
      );
      throw error;
    }
  }
...
When("I convert CSV data to JSON format", async function () {
  ...
  try {
    ...
    logger.info(
      `✅ Converted ${currentData.length} CSV records to JSON format`
    );
    expect(convertedData.length).toEqual(currentData.length);
  } catch (error: any) {
    logger.error(
      `❌ Failed to convert data: ${error?.message || "Unknown error"}`
    );
    throw error;
  }
});
...
When("I filter data by test type {string}", async function (testType: string) {
  ...
  try {
    ...
    logger.info(
      `✅ Filtered ${filteredData.length} records matching test type '${testType}'`
    );
    expect(filteredData.length).toBeGreaterThan(0);
  } catch (error: any) {
    logger.error(
      `❌ Failed to filter data: ${error?.message || "Unknown error"}`
    );
    throw error;
  }
});
...
Then("only smoke test records should be returned", function () {
  ...
  for (const record of filteredData) {
    const isSmoke =
      record.testType === "smoke" || record.testCategory === "smoke";
