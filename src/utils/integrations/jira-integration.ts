...
export class JiraIntegration {
        ...
        try {
            if (existsSync(this.configPath)) {
                ...
                this.config = {
                    ...
                    testExecutionIssueType: process.env.JIRA_TEST_EXECUTION_TYPE || 'Test Execution',
    ...
    private setupInterceptors(): void {
        ...
        this.client.interceptors.response.use(
            ...
            (error) => {
                console.error('Jira API Response Error:', error.response?.data || getErrorMessage(error));
                return Promise.reject(error);
            }
        );
    }
    ...
    async createTestIssue(
        ...
        try {
            ...
            const response: AxiosResponse = await this.client.post('/rest/api/2/issue', issueData);
        ...
        try {
            const issueData = {
                fields: {
                    ...
                    assignee: executionData.assignee ? { name: executionData.assignee } : null,
                    fixVersions: executionData.version ? [{ name: executionData.version }] : [],
            ...
            throw new Error(`Failed to create test execution: ${getErrorMessage(error)}`);
        }
    }
    ...
    async updateTestResults(testResults: JiraTestResult[]): Promise<void> {
        try {
            ...
            throw new Error(`Failed to update test results: ${getErrorMessage(error)}`);
        }
    }
    ...
    private async updateSingleTestResult(result: JiraTestResult): Promise<void> {
        try {
            ...
            const executionData = {
                ...
                defects: result.status === 'FAIL' ? await this.createDefect(result) : []
            };
            ...
            console.error(`Failed to update test result for ${result.testKey}:`, getErrorMessage(error));
        }
    }
     ...
     */
    private async updateXrayTestResult(testKey: string, executionData: any): Promise<void> {
        try {
            const xrayData = {
                testKey: testKey,
            ...
            throw new Error(`Failed to update Xray test result: ${getErrorMessage(error)}`);
        }
    }
     ...
     */
    private async linkTestsToExecution(executionKey: string, testResults: JiraTestResult[]): Promise<void> {
        try {
            for (const result of testResults) {
                await this.linkIssues(executionKey, result.testKey, 'tests');
            }
        } catch (error) {
            console.error('Failed to link tests to execution:', getErrorMessage(error));
        }
    }
     ...
     */
    private async linkIssues(inwardIssue: string, outwardIssue: string, linkType: string): Promise<void> {
        try {
            const linkData = {
                type: { name: linkType },
            ...
            console.error(`Failed to link issues ${inwardIssue} -> ${outwardIssue}:`, getErrorMessage(error));
        }
    }
    ...
    async getIssue(issueKey: string): Promise<JiraIssue> {
        try {
            ...
            throw new Error(`Failed to get issue ${issueKey}: ${getErrorMessage(error)}`);
        }
    }
    ...
    async addComment(issueKey: string, comment: string): Promise<void> {
        try {
            ...
            await this.client.post(`/rest/api/2/issue/${issueKey}/comment`, commentData);
        } catch (error) {
            console.error(`Failed to add comment to ${issueKey}:`, getErrorMessage(error));
        }
    }
     ...
     */
    async uploadAttachments(issueKey: string, attachmentPaths: string[]): Promise<void> {
        try {
            ...
            for (const path of attachmentPaths) {
                if (existsSync(path)) {
                    ...
                    await this.client.post(`/rest/api/2/issue/${issueKey}/attachments`, form, {
                        headers: {
                            ...form.getHeaders(),
            ...
            console.error(`Failed to upload attachments to ${issueKey}:`, getErrorMessage(error));
        }
    }
    ...
    private formatResultComment(result: JiraTestResult): string {
        const status = result.status === 'PASS' ? '✅ PASSED' : result.status === 'FAIL' ? '❌ FAILED' : '⏭️ SKIPPED';
...
${result.comment ? `*Notes:* ${result.comment}` : ''}
     ...
     */
    async searchIssues(jql: string, fields: string[] = ['key', 'summary', 'status']): Promise<JiraIssue[]> {
        try {
    ...
    private async getLinkedTests(executionKey: string): Promise<JiraIssue[]> {
        try {
            const response = await this.client.get(`/rest/api/2/issue/${executionKey}?expand=issuelinks`);
            ...
            for (const key of testKeys) {
                try {
                    ...
                    console.error(`Failed to get linked test ${key}:`, getErrorMessage(error));
                }
            }
