import { JiraIntegration, JiraTestResult, JiraTestExecution } from './jira-integration';
...
export class TestManagementIntegration {
    ...
    constructor(config?: TestManagementConfig) {
        this.configPath = resolve(__dirname, '../../config/test-management-config.json');
            ...
            if (existsSync(this.configPath)) {
                ...
                this.config = {
                    ...
                    testrail: {
                        ...
                        config: process.env.TESTRAIL_ENABLED === 'true' ? undefined : undefined
                    },
                    defaultTool: (process.env.DEFAULT_TEST_TOOL as 'jira' | 'testrail' | 'both') || 'jira'
                };
            }
        } catch (error) {
            console.warn('Failed to load test management configuration, using defaults:', getErrorMessage(error));
            this.config = {
                jira: { enabled: false },
    ...
    private initializeIntegrations(): void {
        try {
            ...
            if (this.config.testrail?.enabled) {
                this.testRailIntegration = new TestRailIntegration(this.config.testrail.config);
            }
        } catch (error) {
            console.error('Failed to initialize test management integrations:', getErrorMessage(error));
        }
    }
    ...
    async testConnections(): Promise<{ jira?: boolean; testrail?: boolean }> {
        ...
        if (this.testRailIntegration) {
            try {
                ...
                console.error('TestRail connection test failed:', getErrorMessage(error));
    ...
    async createTestExecution(
        ...
        if (this.jiraIntegration) {
            try {
                ...
                executionIds.jira = await this.jiraIntegration.createTestExecution(jiraExecution);
            } catch (error) {
                console.error('Failed to create Jira test execution:', getErrorMessage(error));
            }
        }
        ...
        if (this.testRailIntegration) {
            try {
                ...
                if (testRailResults.length > 0) {
                    await this.testRailIntegration.addTestResults(run.id, testRailResults);
                }
            } catch (error) {
                console.error('Failed to create TestRail test run:', getErrorMessage(error));
            }
        }
    ...
    async updateResultsFromCucumberReport(
        ...
        options: {
            ...
            testCaseMapping?: { [scenarioName: string]: { jiraKey?: string; testRailId?: number } };
            version?: string;
            environment?: string;
            executedBy?: string;
        } = {}
        ...
        try {
            ...
            for (const feature of reportData) {
                for (const element of feature.elements) {
                    if (element.type === 'scenario') {
                        ...
                        const status = element.result?.status === 'passed' ? 'PASS' :
                                     element.result?.status === 'failed' ? 'FAIL' : 'SKIP';
            ...
            if (this.jiraIntegration && options.jiraExecutionKey && testResults.length > 0) {
                const jiraResults: JiraTestResult[] = testResults.map(result => ({
                    testKey: result.testKey,
                    status: result.status,
                    executionTime: result.executionTime,
            ...
            if (this.testRailIntegration && options.testRailRunId && testResults.length > 0) {
                const testRailResults: TestRailResult[] = testResults
                ...
                if (testRailResults.length > 0) {
                    await this.testRailIntegration.addTestResults(options.testRailRunId, testRailResults);
                }
            }

            console.log(`Updated ${testResults.length} test results in test management tools`);
        } catch (error) {
    ...
    generateExecutionSummary(
        ...
        const totalExecutionTime = testResults.reduce((total, r) => total + r.executionTime, 0);
    ...
    async generateTestReport(
        ...
        try {
            ...
            throw new Error(`Failed to generate test report: ${getErrorMessage(error)}`);
        }
    }
    ...
    private generateTestComment(element: any): string {
        ...
        if (element.steps && element.steps.length > 0) {
            ...
            element.steps.forEach((step: any, index: number) => {
                ...
                const emoji = stepStatus === 'passed' ? '✅' : stepStatus === 'failed' ? '❌' : '⏭️';
    ...
    async syncTestCases(
        ...
        try {
            if (!this.jiraIntegration || !this.testRailIntegration) {
                throw new Error('Both Jira and TestRail integrations must be enabled for synchronization');
            }

            // Get test cases from Jira
            const jiraTestCases = await this.jiraIntegration.getTestCases(jiraProjectKey);
            
            // Get existing TestRail cases
            const testRailCases = await this.testRailIntegration.getTestCases();
            const testRailCasesByTitle = new Map(testRailCases.map(c => [c.title, c]));

            for (const jiraCase of jiraTestCases) {
                try {
                    const existingTestRailCase = testRailCasesByTitle.get(jiraCase.summary);
                    ...
                    errors.push(`Failed to sync case ${jiraCase.key}: ${getErrorMessage(error)}`);
                }
            }
    ...
    exportConfig(): void {
        try {
            ...
            console.log(`Test management configuration exported to: ${this.configPath}`);
        } catch (error) {
