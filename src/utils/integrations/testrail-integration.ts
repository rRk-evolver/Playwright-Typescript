...
export class TestRailIntegration {
  ...
  private setupInterceptors(): void {
    this.client.interceptors.request.use(
      (config) => {
        console.log(
          `TestRail API Request: ${config.method?.toUpperCase()} ${config.url}`
        );
        return config;
      },
      (error) => {
    ...
    this.client.interceptors.response.use(
      (response) => {
        console.log(
          `TestRail API Response: ${response.status} ${response.statusText}`
        );
        return response;
      },
      (error) => {
        console.error(
          "TestRail API Response Error:",
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.client.get(
        "/get_user_by_email&email=" + this.config.username
      );
  ...
  async createTestCase(
    ...
    try {
      ...
      if (template) {
        if (template.steps) {
          caseData.custom_steps_separated = template.steps.map(
            (step, index) => ({
        ...
        if (template.preconditions)
          caseData.custom_preconds = template.preconditions;
  ...
  async getTestRuns(
    ...
    try {
      ...
      if (isCompleted !== undefined)
        params.append("is_completed", isCompleted ? "1" : "0");
  ...
  async createMilestone(
    ...
    try {
      ...
      const response = await this.client.post(
        `/add_milestone/${this.config.projectId}`,
        milestoneData
      );
      return response.data;
    } catch (error) {
    ...
    try {
        ...
        testDetails: tests.map((test) => {
          ...
          return {
            ...
            status: this.getStatusName(
              result?.status_id || TestRailIntegration.STATUS.UNTESTED
            ),
      ...
      report.executionSummary.averageExecutionTime =
        report.completedTests > 0
          ? Math.round(
              report.executionSummary.totalExecutionTime / report.completedTests
            )
          : 0;
   ...
   */
  async createTestCaseMappingFromFeatures(
    featuresPath: string,
    sectionId: number
  ): Promise<{ [scenarioName: string]: number }> {
    try {
      ...
      throw new Error(
        `Failed to create test case mapping: ${getErrorMessage(error)}`
      );
    }
  }
