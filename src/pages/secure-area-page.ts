...
export class SecureAreaPage extends BasePage {
  ...
  async navigate(): Promise<void> {
    ...
    const secureUrl =
      process.env.BASE_URL?.replace("/todomvc", "") + "/secure" ||
  ...
  async verifySecureContent(): Promise<void> {
    ...
    if (!hasSuccessMessage) {
      logger.warn(
        "Success message not found, but user appears to be authenticated"
      );
    }
    ...
    if (!canLogout) {
      throw new Error(
        "Logout button not available - user may not be properly authenticated"
      );
    }
