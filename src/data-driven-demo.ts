  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(csvPath)) {
        ...
        // Test filtering
        const validUsers = csvData.filter(
          (record: any) => record.expectedResult === "success"
        );
    ...
    try {
      ...
      if (fs.existsSync(excelPath)) {
        const excelData = await excelHandler.readExcelFile(
          excelPath,
          "UserData"
        );
        ...
        if (sheetNames.includes("TestCases")) {
          const testCases = await excelHandler.readExcelFile(
            excelPath,
            "TestCases"
          );
    ...
    try {
      ...
      if (fs.existsSync(jsonPath)) {
        ...
        const baseUrl = await jsonHandler.getProperty(
          jsonPath,
